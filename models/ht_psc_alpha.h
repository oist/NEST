...
#ifndef HT_PSC_ALPHA_H
#define HT_PSC_ALPHA_H
...
namespace nest{
   ...
   */
  extern "C"
  int ht_psc_alpha_dynamics(double, const double*, double*, void*);

 /* BeginDocumentation
Name: ht_psc_alpha - Hodgkin Huxley neuron model.

Description:

  ht_psc_alpha is an implementation of a spiking neuron using the Hodkin-Huxley formalism.
...
*/
  ...
  public:        
    ...
    ht_psc_alpha();
    ht_psc_alpha(const ht_psc_alpha&);
    ~ht_psc_alpha();
  ...
  private:
    ...
    friend int ht_psc_alpha_dynamics(double, const double*, double*, void*);

    // The next two classes need to be friend to access the State_ class/member
    friend class RecordablesMap<ht_psc_alpha>;
    friend class UniversalDataLogger<ht_psc_alpha>;
  ...
  private:
    ...
    struct Parameters_ {
      ...
      double_t g_HT;  			//!< High-threshold T type Calcium Conductance in nS
      double_t g_LT;  			//!< Low-threshold T type Calcium Conductance in nS
      double_t g_H;  			//!< H Conductance in nS
      double_t g_AHP;  			//!< AHP Conductance in nS
      double_t g_L;       //!< Leak Conductance in nS
      double_t g_KL;       //!< Leak Conductance in nS
      ...
      double_t E_H;	  		//!< H Reversal Potential in mV
      double_t E_AHP;	  		//!< AHP Reversal Potential in mV
  ...
  private:
     ...
     */
    struct Buffers_ {
      Buffers_(ht_psc_alpha&);                   //!<Sets buffer pointers to 0
      Buffers_(const Buffers_&, ht_psc_alpha&);  //!<Sets buffer pointers to 0

      //! Logger for all analog data
      UniversalDataLogger<ht_psc_alpha> logger_;
    ...
    static RecordablesMap<ht_psc_alpha> recordablesMap_;
  };

  inline
  port ht_psc_alpha::send_test_event(Node& target, rport receptor_type, synindex, bool) 
  { 
    SpikeEvent e;
    e.set_sender(*this); 
  ...
  inline
  port ht_psc_alpha::handles_test_event(SpikeEvent&, rport receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port ht_psc_alpha::handles_test_event(CurrentEvent&, rport receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port ht_psc_alpha::handles_test_event(DataLoggingRequest& dlr, 
				    rport receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void ht_psc_alpha::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  inline
  void ht_psc_alpha::set_status(const DictionaryDatum &d)
  {
