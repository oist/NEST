...
namespace nest   // template specialization must be placed in namespace
{
  ...
  void RecordablesMap<iaf_cond_beta>::create()
  {
    ...
    insert_(names::V_m, 
	    &iaf_cond_beta::get_y_elem_<iaf_cond_beta::State_::V_M>);
    insert_(names::g_ex, 
...
extern "C"
inline int nest::iaf_cond_beta_dynamics(double, const double y[], double f[], void* pnode)
{ 
  // a shorthand
  ...
  f[2] =  y[S::DG_EXC]*(1.0/node.P_.tau_synE2 -1.0/node.P_.tau_synE1) - (y[S::G_EXC]/node.P_.tau_synE1); 
  f[1] = -y[S::DG_EXC] / node.P_.tau_synE2;
  
  // d dg_exc/dt, dg_exc/dt
  f[4] =  y[S::DG_INH]*(1.0/node.P_.tau_synI2 -1.0/node.P_.tau_synI1) - (y[S::G_INH]/node.P_.tau_synI1); 
  f[3] = -y[S::DG_INH] / node.P_.tau_synI2;
...
void nest::iaf_cond_beta::Parameters_::get(DictionaryDatum &d) const
{
  ...
  def<double>(d,"tau_syn_ex1",   tau_synE1);
  def<double>(d,"tau_syn_ex2",   tau_synE2);
  def<double>(d,"tau_syn_in1",   tau_synI1);
  def<double>(d,"tau_syn_in2",   tau_synI2);
  def<double>(d,names::I_e,          I_e);
}

void nest::iaf_cond_beta::Parameters_::set(const DictionaryDatum& d)
{
  ...
  updateValue<double>(d,"tau_syn_ex1", tau_synE1);
  updateValue<double>(d,"tau_syn_ex2", tau_synE2);
  updateValue<double>(d,"tau_syn_in1", tau_synI1);
  updateValue<double>(d,"tau_syn_in2", tau_synI2);
...
void nest::iaf_cond_beta::State_::get(DictionaryDatum &d) const
{
  def<double>(d, names::V_m, y[V_M]); // Membrane potential
}

void nest::iaf_cond_beta::State_::set(const DictionaryDatum& d, const Parameters_&)
{
...
void nest::iaf_cond_beta::init_state_(const Node& proto)
{
  const iaf_cond_beta& pr = downcast<iaf_cond_beta>(proto);
  S_ = pr.S_;
}

void nest::iaf_cond_beta::init_buffers_()
{
...
void nest::iaf_cond_beta::calibrate()
{
  ...
  V_.PSConInit_E  = 1.0/(std::pow(P_.tau_synE2/P_.tau_synE1, P_.tau_synE2/(P_.tau_synE1 - P_.tau_synE2)) -
			 std::pow(P_.tau_synE2/P_.tau_synE1, P_.tau_synE1/(P_.tau_synE1 - P_.tau_synE2)));

  V_.PSConInit_I  = 1.0/(std::pow(P_.tau_synI2/P_.tau_synI1, P_.tau_synI2/(P_.tau_synI1 - P_.tau_synI2)) -
			 std::pow(P_.tau_synI2/P_.tau_synI1, P_.tau_synI1/(P_.tau_synI1 - P_.tau_synI2)));
...
void nest::iaf_cond_beta::update(Time const & origin, const long_t from, const long_t to)
{
...
void nest::iaf_cond_beta::handle(SpikeEvent & e)
{
...
void nest::iaf_cond_beta::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void nest::iaf_cond_beta::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
