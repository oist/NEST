...
#ifndef TC_PSC_ALPHA_H
#define TC_PSC_ALPHA_H
...
namespace nest{
   ...
   */
  extern "C"
  int tc_psc_alpha_dynamics(double, const double*, double*, void*);

 /* BeginDocumentation
Name: tc_psc_alpha - Hodgkin Huxley neuron model.

Description:

  tc_psc_alpha is an implementation of a spiking neuron using the Hodkin-Huxley formalism.
...
*/
  ...
  public:        
    ...
    tc_psc_alpha();
    tc_psc_alpha(const tc_psc_alpha&);
    ~tc_psc_alpha();
  ...
  private:
    ...
    friend int tc_psc_alpha_dynamics(double, const double*, double*, void*);

    // The next two classes need to be friend to access the State_ class/member
    friend class RecordablesMap<tc_psc_alpha>;
    friend class UniversalDataLogger<tc_psc_alpha>;
  ...
  private:
    ...
    struct Parameters_ {
      ...
      double_t g_T;  			//!< T type Calcium Conductance in nS
      ...
      double_t E_KL;	  		//!< Potassium Reversal Potential in mV
      double_t E_H;	  		//!< Potassium Reversal Potential in mV
  ...
  private:
     ...
     */
    struct Buffers_ {
      Buffers_(tc_psc_alpha&);                   //!<Sets buffer pointers to 0
      Buffers_(const Buffers_&, tc_psc_alpha&);  //!<Sets buffer pointers to 0

      //! Logger for all analog data
      UniversalDataLogger<tc_psc_alpha> logger_;
  ...
  inline
  void tc_psc_alpha::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  inline
  void tc_psc_alpha::set_status(const DictionaryDatum &d)
  {
