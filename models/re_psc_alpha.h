...
#ifndef RE_PSC_ALPHA_H
#define RE_PSC_ALPHA_H
...
namespace nest{
   ...
   */
  extern "C"
  int re_psc_alpha_dynamics(double, const double*, double*, void*);

 /* BeginDocumentation
Name: re_psc_alpha - Hodgkin Huxley neuron model.

Description:

  re_psc_alpha is an implementation of a spiking neuron using the Hodkin-Huxley formalism.
...
*/
  ...
  public:        
    ...
    re_psc_alpha();
    re_psc_alpha(const re_psc_alpha&);
    ~re_psc_alpha();
  ...
  private:
    ...
    friend int re_psc_alpha_dynamics(double, const double*, double*, void*);

    // The next two classes need to be friend to access the State_ class/member
    friend class RecordablesMap<re_psc_alpha>;
    friend class UniversalDataLogger<re_psc_alpha>;
  ...
  private:
    ...
    struct Parameters_ {
      ...
      double_t g_K_AHP;  		//!< Slow Ca-dependent Potassium Conductance in nS
      double_t g_CAN;  		//!< CAN Conductance in nS
      double_t g_Ca;  			//!< Calcium Conductance in nS
      double_t g_L;       //!< Leak Conductance in nS
      double_t g_Ts;      //!< low-threshold Ca2+ Conductance in nS
      ...
      double_t E_Ca;	  		//!< Calcium Reversal Potential in mV
      double_t E_CAN;     //!< CAN Reversal Potential in mV
  ...
  private:
     ...
     */
    struct Buffers_ {
      Buffers_(re_psc_alpha&);                   //!<Sets buffer pointers to 0
      Buffers_(const Buffers_&, re_psc_alpha&);  //!<Sets buffer pointers to 0

      //! Logger for all analog data
      UniversalDataLogger<re_psc_alpha> logger_;
    ...
    static RecordablesMap<re_psc_alpha> recordablesMap_;
  };

  inline 
  port re_psc_alpha::send_test_event(Node& target, rport receptor_type, synindex, bool) 
  { 
    SpikeEvent e;
    e.set_sender(*this); 
  ...
  inline
  port re_psc_alpha::handles_test_event(SpikeEvent&, rport receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port re_psc_alpha::handles_test_event(CurrentEvent&, rport receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port re_psc_alpha::handles_test_event(DataLoggingRequest& dlr, 
				    rport receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void re_psc_alpha::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d);
  ...
  inline
  void re_psc_alpha::set_status(const DictionaryDatum &d)
  {
